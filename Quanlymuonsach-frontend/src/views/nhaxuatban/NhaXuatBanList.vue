            ...
            <tbody>
              ...
              <tr v-else-if="nhaXuatBans.length === 0">
                <td :colspan="6" class="text-center py-5 text-muted">
                  ...
                  <router-link to="/nhaxuatban/add" class="btn btn-primary btn-sm mt-3">
                    <i class="fas fa-plus me-1"></i>Thêm nhà xuất bản đầu tiên
                  </router-link>
                </td>
              </tr>
  ...
  methods: {
    ...
    async handleDelete(id) {
      ...
      try {
        ...
        alert('Lỗi khi xóa: ' + (error.response?.data?.message || error.message || 'Thử lại!'))
      } finally {
        this.loading = false
      }
    },
